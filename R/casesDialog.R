...
function () {
    defaults <- list (initial.year1 = 1, initial.month1 = 5, initial.day1 = 0,
                      ...
                      initial.x = 11, initial.measure = "new_cases", initial.permillion = "FALSE",
                      initial.mean = TRUE, initial.bar=FALSE,initial.atop=FALSE)
    dialog.values <- getDialog ("casesDialog", defaults)
    initializeDialog(title = gettextRcmdr("Covid cases and deaths"))
    ...
    onOK <- function() {
      ...
      putDialog ("casesDialog",list (initial.year1 = year1.i, initial.month1 = month1.i,
      ...
      if(year1!="None" && year2!="None"){
        doItAndPrint(paste0("drawBars(data=dataCovid, countries=",xc,",start='",date1,"',end='",date2,"',measure='",measure,
                            "',perMillion=",permillion,",drawMean=",mean,",bars=",bar,",atop=",atop,")"))}else
                              if (year1!="None"){
                                doItAndPrint(paste0("drawBars(data=dataCovid, countries=",xc,",start='",date1,"',measure='",measure,"',perMillion=",permillion,",drawMean=",mean,",bars=",bar,",atop=",atop,")"))}else
                              if (year2!="None"){
                                    doItAndPrint(paste0("drawBars(data=dataCovid, countries=",xc,", start=NULL, end='",date2,"',measure='",measure,"',perMillion=",permillion,",drawMean=",mean,",bars=",bar,",atop=",atop,")"))}else
                                {
                                  doItAndPrint(paste0("drawBars(data=dataCovid, countries=",xc,", start=NULL, measure='",measure,"',perMillion=",permillion,",drawMean=",mean,",bars=",bar,",atop=",atop,")"))
                                }
      
      tkdestroy(top)
      tkfocus(CommanderWindow())
    }
    OKCancelHelp(helpSubject = "drawBars", reset = "casesDialog", apply = "casesDialog")
    
    measureFrame <- tkframe(middleFrame)
    permillionFrame <- tkframe(rightFrame)
    optionsFrame <- tkframe(rightFrame)
    
    radioButtons(middleFrame, name = "measure", buttons = c("newcases", "totalcases", "newdeaths", 
                                  "totaldeaths","hosppatients","icupatients","CFR","R"), values = c("new_cases","total_cases",
                                  "new_deaths","total_deaths","hosp_patients","icu_patients","case_fatality_rate","reproduction_rate"), 
                 labels = gettextRcmdr(c("New cases", "Total cases", "New deaths", "Total deaths","Hosp. patients","ICU patients",
                                         "Case fatality rate", "Reproduction rate")), 
